...
export default function ContactData({
  ...
  /* Check if the location is valid by regex */
  const isValidLocation = (location) => {
    return /^\d{5}\s(?:[A-ZÄÖÜa-zäöüß\- ]{1,40}){1,5}$/.test(location);
  };
  ...
  /* Error for the location field */
  const pickUpZipWarning = (location) => {
    const newLocation = location.trim();
    let errorText = `Abholung ist leider für deinen PLZ-Bereich nicht möglich (nur für ${officeZipFirstTwoDigits}xxx).
     Bitte Übergabe an der Geschäftsstelle auswählen.`;
  ...
  return (
    <div className="contact-data">
      ...
      <label className={`mobile-hidden ${isHandover ? "hidden" : ""} `}>
        ...
        {!locationError && locationZipWarning && (
          <p className="errorText">{locationZipWarning}</p>
        )}
        {!locationZipWarning && !isPickupZip(location) && (
