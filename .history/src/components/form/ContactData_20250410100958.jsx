...
export default function ContactData({
  ...
  const isEmpty = (name) => newName.length === 0;

  /* Check name by word count */
  const wordCount = newName.trim().split(" ").length;
  ...
  const handleNameChange = (e) => {
    ...
    const isValidName = /^[A-Za-zÄÖÜäöüß\s\-']+$/.test(newName);
    if (isValidName) {
      setContactData({
        ...contactData,
        name: newName,
