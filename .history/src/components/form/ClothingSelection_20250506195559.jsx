...
export default function ClothingSelection({
  ...
  const { name, nameError } = contactData;
  ...
  useEffect(() => {
    if (name.trim() !== "" && !nameError && clothingItems.length == 0) {
      setClothingSelection((prev) => ({
        ...prev,
        clothingItemError: true,
        clothingItemErrorText: "Bitte wähle mind. eine Kleidungsart aus",
      }));
    }
  });
