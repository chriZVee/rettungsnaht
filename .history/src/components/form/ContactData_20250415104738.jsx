...
export default function ContactData({
  ...
  const pickUpNote = (location) => {
    const newLocation = location.trim();
    let error = validateLocation(newLocation);

    if (!isPickupZip(newLocation) && error !== true) {
      let text = `Abholung nur für Plz-Bereich ${officeZipFirstTwoDigits}xxx
      verfügbar.`;
  ...
  return (
    <div className="contact-data">
      ...
      <label className={`mobile-hidden ${isHandover ? "hidden" : ""} `}>
        ...
        {<p className="infoText">{pickUpNote(location)}</p>}
      </label>
      {console.log(validateLocation(location).error === false)}
    </div>
  );
}
