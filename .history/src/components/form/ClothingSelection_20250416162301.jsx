...
export default function ClothingSelection({
  ...
  const [itemsNoticeText, setItemsNoticeText] = useState(
    "Bitte wähle mind. eine Kleidungsart aus."
  );
  const [itemsNotice, setItemsNotice] = useState(true);
  ...
  return (
    <fieldset className="clothing-selection">
      ...
      <div className="clothing-selection-container">
        ...
        {!clothingItemError && itemsNotice && (
          <p className="noticeText">{itemsNoticeText}</p>
        )}
      </div>
    </fieldset>
  );
