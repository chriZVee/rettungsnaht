...
export default function ContactData({
  ...
  const pickUpZipWarning = (location) => {
    ...
    if (!isPickupZip(newLocation)) {
      return (errorText = `Abholung ist leider für deinen PLZ-Bereich nicht möglich (nur für ${officeZipFirstTwoDigits}xxx). Bitte Übergabe an der Geschäftsstelle auswählen.`);
    } else {
      return "";
    }
  };
  ...
  return (
    <div className="contact-data">
      ...
      <label className={`mobile-hidden ${isHandover ? "hidden" : ""} `}>
        ...
        {locationError && pickUpZipWarning && (
          <p className="errorText">{pickUpZipWarning(location)}</p>
        )}
        {!locationError &&
          pickUpZipNote(<p className="infoText">{pickUpZipNote(location)}</p>)}
      </label>
    </div>
  );
}
