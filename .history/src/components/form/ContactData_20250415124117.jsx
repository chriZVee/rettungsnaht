...
export default function ContactData({
  ...
  const pickUpZipWarning = (location) => {
    const newLocation = location.trim();
    let pickUpZipWarning = `Abholung ist leider für deinen PLZ-Bereich nicht möglich (nur für ${officeZipFirstTwoDigits}xxx). Bitte Übergabe an der Geschäftsstelle auswählen.`;

    if (!isPickupZip(newLocation)) {
      return pickUpZipWarning;
    } else {
      return "";
    }
  };
  ...
  const pickUpZipNote = (location) => {
    ...
    if (pickUpZipWarning(newLocation) !== "" && !isEmpty(newLocation)) {
  ...
  return (
    <div className="contact-data">
      ...
      <label className={`mobile-hidden ${isHandover ? "hidden" : ""} `}>
        ...
        {pickUpZipWarning && !isEmpty(location) && (
          <p className="pickUpZipWarning">{pickUpZipWarning(location)}</p>
        )}
        {isEmpty(location) && pickUpZipNote && (
